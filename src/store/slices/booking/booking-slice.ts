...
import { createBookingWithBookPage, createBookingWithMainPage, deleteBooking, editBooking } from './async-actions';
...
export const bookingSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(createBookingWithMainPage.pending, (state) => {
      state.error = null;
      ...
      builder.addCase(createBookingWithMainPage.fulfilled, (state, action) => {
      ...
      builder.addCase(createBookingWithMainPage.rejected, (state, action) => {
      ...
      builder.addCase(createBookingWithBookPage.pending, (state) => {
        state.error = null;
        state.status = RequestStatusType.PENDING;
        state.response = null;
        state.type = CrudActionTypes.CREATE;
        ...
        builder.addCase(createBookingWithBookPage.fulfilled, (state, action) => {
          state.status = RequestStatusType.FULFILLED;
          state.error = null;
          // state.response = action.payload
          state.textStatusForAlert = 'Книга забронирована. Подробности можно посмотреть на странице Профиль';
        ...
        builder.addCase(createBookingWithBookPage.rejected, (state, action) => {
          state.status = RequestStatusType.REJECTED;
          state.error = JSON.parse(String(action.payload));
          state.response = null;
          state.textStatusForAlert = 'Что-то пошло не так, книга не забронирована. Попробуйте позже!';
